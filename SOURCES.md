# 📊 Sources & Real-World Formats

This document outlines the research behind the three source types, the ingestion format chosen, the sample data generated, and the potential failure modes.
...
* **Format Chosen:** Flat File (CSV) export via a standard SAP GUI report.
* **Why:** It is the most common format a sustainability manager can extract without needing SAP Basis admin privileges or API keys.
...
* **Failure Modes:** In a real deployment, a client might have a totally different localization setting, exporting dates in yet another format, or using custom Z-tables that break standard mapping. The ingestion script handles many date formats, but would need a robust dynamic mapping configuration layer in production.
...
* **Why:** Parsing PDFs via OCR is brittle and error-prone. EDI is complex. Most modern utilities allow downloading billing history to a CSV, but they often attach unparseable metadata/logos to the top of the file.
...
* **Failure Modes:** Real utility data often contains estimated readings vs. actual readings. If a bill is later corrected by the utility, our system would currently ingest the correction as a duplicate rather than a retroactive update to the original record.

---

## 3. Corporate Travel (Concur / Navan)

* **Research:** Platforms like Concur provide an API (Expense API / Itinerary API), but many companies just run standard reporting extracts. Travel data usually contains segments (Origin, Destination), distances, and service classes (Economy, Business).
* **Format Chosen:** Pipe-Separated Values (PSV) Report Export.
* **Why:** Similar to SAP, an API integration requires OAuth credentials and IT approvals. A text export from Concur is universally accessible, but often uses weird delimiters to avoid clashing with commas inside addresses.
